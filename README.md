...
The aim of the project is to develop state-of-the-art deep learning models to detect hand keypoints from 2D images. Once we have an accurate hand keypoint detection model, we will then develop an automatic, robust and objective system for MDS-UPDRS (for Parkinson and movement disorder) hand movement examination, which roughly consists of three major modules, 1) hand detection and pose estimation; -> 2) movement pattern extraction and signal analysis; -> 3) MDS-UPDRS prediction. An illustration of the system is shown below (please don't send it to the students since it is an updating figure from our manuscript)
